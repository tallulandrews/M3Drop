...
<<>>=
head(marker_genes[marker_genes$Group==3,],20) 
...
This difference can also be seen by comparing the heatmaps for the respective genes (Figure 3,5). The highly variable genes contains many more genes exhibiting noisy expression, whereas nearly all genes detected by M3Drop are clearly differentially expressed across the different cell populations. 
