---
title: 'M3Drop: How to model dropouts in scRNASeq using Michaelis-Menten'
...
```{r eval=FALSE}
install.package('M3Drop_0.99.0.tar.gz')
```
...
```{r}
Normalized_data = M3Drop_Clean_Data(Mmus_example_list$data, labels = Mmus_example_list$labels, is.counts=TRUE, min_detected_genes=2000) # Remove cells with <2,000 detected genes and apply CPM normalization
...
```{r}
heat_out = M3Drop_Expression_Heatmap(DE_genes$Gene, Normalized_data$data, cell_labels = Normalized_data$labels)
```
...
For comparison purposes, I have also included a function which implements the method to identify highly variable genes presented in Brennecke et al. (2013) with the added option run it without providing spike-ins, in which case all genes are used to fit the function between CV2 and mean expression.
...
```{r}
heat_out = M3Drop_Expression_Heatmap(HVG, Normalized_data$data, cell_labels = Normalized_data$labels)
```
